...
public class ContextClassGenerator {
	...
	private static void generateTestClasses(Map<String, RuleType> rules) throws FileNotFoundException {
		...
		STGroupFile stgf = new STGroupFile("templates/code_coverage/ClassContext.stg");
		...
		for(Entry<String, RuleType> e:rules.entrySet()){
			if(! e.getValue().isAlt()){
				...
				FileUtils.WriteFile(new File("src/com.proteus.parser/code_coverage/ext/"+classname+"Ext.java"), st.render());
